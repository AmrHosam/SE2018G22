...
if ($_SESSION['new_user'] === true) { //checks if the user already has an account to avoid entering duplicate users
    ...
    $stmt1 = $link->prepare("INSERT INTO `users` (student_id, name_ar,name_en,`address`,mobile_number,
...
$stmt2 = $link->prepare("INSERT INTO `requests` (student_id, `type`, reason) VALUES (?,?,?)");
/************************************************************************/
if (false === $stmt1) {
...
$rc2 = $stmt2->bind_param('isi', $student_id, $type, $reason);
/************************************************************************/
if (false === $rc1) {
