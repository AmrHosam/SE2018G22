...
if(isset($_POST['message']) && $_POST['message'] != '') {
	$sql = "INSERT INTO message(chat_id, user_id, user_name, message, post_time) VALUES (" . 
			mysqli_real_escape_string($link,$_POST['chat']) . ",".mysqli_real_escape_string($link,$_POST['chat'])." , '" . mysqli_real_escape_string($link,$_POST['name']) . 
...
if(!isset($_GET['chat'])) {
	...
	$sql = "SELECT message_id, user_name, message, date_format(post_time, '%h:%i') as post_time" . 
		" FROM message WHERE user_id = " . mysqli_real_escape_string($link,$_GET['chat']) . " AND message_id > " . $last;
